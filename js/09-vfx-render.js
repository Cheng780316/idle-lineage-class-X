...
function _playerBattleForm() {   // 主玩家戰場形態：變身優先 → 職業動態 → null（變身限定主玩家·隊員一律職業動畫）
    ...
    if (m) {
        ...
        let folder = LV90_MORPH_BATTLE_TUNING[m]
            ...
            : m;
        return { key: 'morph:' + folder, domKey: 'morph:' + m, base: 'assets/morphanim/' + encodeURIComponent(folder) + '/', wpn: null, morphName: m,
            assetRev: LV90_MORPH_BATTLE_TUNING[m] ? '20260717-lv90-3dir1' : null };
    }
...
// Lv.90 五款變身的原始素材是 3 倍畫布，而且每個動作的畫布尺寸不同。
// 以待機可見身高約 70px 為基準縮放；實際定位由每一幀的 alpha 腳底中心決定，避免攻擊時整張圖跳位。
...
function _applyLv90MorphFrameGeometry(st, form, frame, actor) {
    ...
    let b = _visibleImageBounds(frame) || { x0:0, y0:0, x1:1, y1:1 };
    let nw = frame.naturalWidth || 1, nh = frame.naturalHeight || 1, scale = cfg.scale;
    let footCx = (b.footX != null ? b.footX : (b.x0 + b.x1) * 0.5) * nw * scale;
    let bottomPad = (1 - b.y1) * nh * scale;
    let img = st.imgs.bd;
