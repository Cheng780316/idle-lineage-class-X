...
function _playerBattleForm() {   // 主玩家戰場形態：變身優先 → 職業動態 → null（變身限定主玩家·隊員一律職業動畫）
    ...
    if (m) return { key: 'morph:' + m, domKey: 'morph:' + m, base: 'assets/morphanim/' + encodeURIComponent(m) + '/', wpn: null, morphName: m };
...
// d6 原始方向朝畫面左上；目標在角色右側時水平鏡射。保留上一方向可避免目標接近正上方時左右抖動。
function _lv90MorphFlip(actor, el) {
    try {
        let uid = actor && actor._faceTgtUid;
        let tr = (uid != null && typeof _vfxSlotRect === 'function') ? _vfxSlotRect(uid) : null;
        let er = el && el.getBoundingClientRect ? el.getBoundingClientRect() : null;
        if (!tr || !tr.width || !er) return !!(actor && actor._lv90MorphFlipX);
        let dx = (tr.left + tr.width * 0.5) - (er.left + er.width * 0.5);
        let deadZone = Math.max(12, tr.width * 0.22);
        if (Math.abs(dx) > deadZone) actor._lv90MorphFlipX = dx > 0;
        return !!actor._lv90MorphFlipX;
    } catch (e) { return !!(actor && actor._lv90MorphFlipX); }
}
function _applyLv90MorphFrameGeometry(st, form, frame, actor) {
    ...
    let flipX = _lv90MorphFlip(actor, st.el);
    let img = st.imgs.bd;
    st.el.style.width = '1px';
    img.style.width = (nw * scale) + 'px';
    img.style.height = (nh * scale) + 'px';
    img.style.left = (-footCx) + 'px';
    img.style.bottom = (-bottomPad) + 'px';
    img.style.transformOrigin = footCx + 'px 50%';
    img.style.transform = flipX ? 'scaleX(-1)' : 'none';
    st.flipX = flipX; st.lv90Tuned = true;
    return true;
}
