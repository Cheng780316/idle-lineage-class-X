...
const MORPH_BATTLE_ANIM = new Set(['克特', '卡司特王', '思克巴女皇', '死亡騎士', '炎魔', '白金法師', '白金騎士', '艾莉絲', '銀光法師', '銀光騎士', '騎士范德', '黃金法師', '黃金騎士', '黑暗法師', '黑暗騎士',
    ...
    '莉絲安']);   // Lv90 五種職業變身＋🏹 Lv80 莉絲安（透明逐幀動畫）
...
function _actorBattleForm(actor, allyGrp) {   // 玩家／傭兵戰場形態：變身優先 → 職業動態 → null
    // 🧝 真夏納（classMorph）：職業式變身走 _classForm 管線但用專屬資料夾。
    let _pf = (typeof getActivePolyForm === 'function') ? getActivePolyForm(actor) : (actor && (actor._setPoly || ((actor.buffs && actor.buffs.poly > 0 && actor.poly) ? actor.poly : null)));
    if (_pf && _pf.classMorph && actor && actor.avatar && TRUE_SHANNA_ANIM_SETS[actor.avatar]) {
        let cf = _classForm(actor, !!allyGrp, _pf.n);
        if (cf) return cf;
    }
    let m = _actorMorphName(actor);
    if (m) {
        // Lv.90 五款變身依玩家指定，比照 80 級死亡騎士固定朝右上（客戶端 d2）。
        // 不再依目標切換三方向，避免待機與攻擊時產生像走動、轉身或位置跳動的畫面。
        let fixedLv90 = !!LV90_MORPH_BATTLE_TUNING[m];
        let face = fixedLv90 ? 'R' : ((actor && actor._face3) || 'R');
        let folder = (!fixedLv90 && MORPH_ANIM_3DIR.has(m)) ? (face === 'F' ? m + 'F' : face === 'L' ? m + '2' : m) : m;
...
function _actorMorphName(actor) {   // 玩家／傭兵目前變身名（含套裝別名映射）·沒有動畫資源則退回職業外觀
    ...
    let f = (typeof getActivePolyForm === 'function') ? getActivePolyForm(actor) : (actor._setPoly || ((actor.buffs && actor.buffs.poly > 0 && actor.poly) ? actor.poly : null));
...
// Lv.90 原始素材最長有 17 幀；各變身可在 tuning 指定攻擊關鍵幀，避免 8fps 更新時錯過主要揮擊。
// 其餘較長動作仍允許播放至 18ms/幀，確保高速攻擊能在下一擊前完成。
...
function _playerMorphApply() {   // 8fps ticker 驅動（🗡️ v3.0.67 形態＝變身優先→職業動態·key 含武器變體→換武器自動重建）
    ...
    let _activeMorphName = _playerMorphName() || '';
    if (!LV90_MORPH_BATTLE_TUNING[_activeMorphName] && (CLASS_ANIM_3DIR.has(player.avatar) || MORPH_ANIM_3DIR.has(_activeMorphName))) _class3Facing(player, _pmState.el);   // 🧭 一般職業／變身依目標換向；Lv.90 變身固定右上
