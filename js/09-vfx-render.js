...
//     尺寸二選一：h=特效高度為目標圖高倍數(投射型如落雷)／w=特效寬度為目標圖寬倍數(地面型如地裂術·寬≈怪寬)，另一維依原始比例推導；
...
const SPELL_FX = {
    ...
    '火箭':     { dir: '火箭',     prefix: '171-1', n: 5,  fps: 12, blend: 'screen', h: 1.0, ax: 0.50, ay: 0.55 },   // gfx171 fireball：火球衝擊·加亮·目標身上
    '冰箭':     { dir: '冰箭',     prefix: '167-1', n: 4,  fps: 12, blend: 'screen', h: 0.9, ax: 0.50, ay: 0.55 },   // gfx167 energy bolt：衝擊·加亮·目標身上
    '光箭':     { dir: '冰箭',     prefix: '167-1', n: 4,  fps: 12, blend: 'screen', h: 0.9, ax: 0.50, ay: 0.55 },   // gfx167 energy bolt：光箭與冰箭同 spr(用戶告知 167 也作用於光箭)→複用已部署 167 幀·不另存檔
    // 🔮 v2.7.39 攻擊/減益型法術特效批次(13 支·來源 spr彙整/fx/·單檔 --luma-alpha 去黑邊·screen 加亮·觸發免寫=js/07 castSkillInner 逐目標迴圈通用·名字對上技能 n 自動播)：
    '冰矛圍籬':   { dir: '冰矛圍籬',   prefix: '1809-2', n: 3,  fps: 10, blend: 'screen', h: 0.90, ax: 0.50, ay: 0.55 },   // v2.7.41 換 gfx1809(原756)·70×48
    '冰錐':       { dir: '冰錐',       prefix: '1797-1', n: 4,  fps: 12, blend: 'screen', h: 0.85, ax: 0.50, ay: 0.55 },   // v2.7.41 冰錐改自有 spr gfx1797(原複用冰矛圍籬)·15×21 小
    ...
    '風刃':       { dir: '風刃',       prefix: '1799-0', n: 5,  fps: 12, blend: 'screen', h: 0.90, ax: 0.50, ay: 0.55 },   // gfx1799·風刃 36×26
...
const MOB_ANIM_DEATH_FX = {
    '冰之女王':     { n: 25, ew: 158, eh: 115, anchored: { ox: -26, oy: 10, bw: 101, bh: 114 } },
...
function playSpellFx(skn, mob) {
    try {
        ...
        let _anc = _mobImgAnchor(box.querySelector('img:not(.mob-anim-shadow):not(.mob-anim-weapon):not(.mob-anim-weapon2)'));
        ...
        let _computeGeom = () => {
            if (cfg.w != null) { fxW = r.width * cfg.w; fxH = fxW / ar; }       // 🌋 寬度驅動(地面型·寬≈怪寬)
            else { fxH = r.height * (cfg.h || 1.8); fxW = fxH * ar; }            // ⚡ 高度驅動(投射型·如落雷)
...
function _updateMobSkillFx() {
    try {
        ...
        for (let uid of ids) {
            ...
            if (!m || !a || a === 'probing' || !a.skillFx || !a.skillFx.start) { if (s.el) s.el.remove(); if (s.el2) s.el2.remove(); delete _mobSkillFx[uid]; continue; }
            ...
            if (f >= total || !card) { if (s.el) s.el.remove(); if (s.el2) s.el2.remove(); delete _mobSkillFx[uid]; continue; }
...
function _renderMobsImpl() {
    ...
    for(let _k = 0; _k < _order.length; _k++) {
        ...
        if (m) {
            ...
            let _shadowLayer = _spriteShadow ? `<img class="mob-anim-shadow w-24 h-24 p-1 object-contain pointer-events-none" src="assets/anim/${m.n}/idle_s_0.png" alt="" aria-hidden="true" onerror="this.style.display='none'">` : '';
            ...
            let _weaponLayer = _weaponFx ? `<img class="mob-anim-weapon w-24 h-24 p-1 object-contain pointer-events-none" src="assets/anim/${m.n}/idle_w_0.png" alt="" aria-hidden="true" onerror="this.style.display='none'">` : '';
            ...
            let _weaponLayer2 = _weaponFx2 ? `<img class="mob-anim-weapon2 w-24 h-24 p-1 object-contain pointer-events-none" src="assets/anim/${m.n}/idle_w2_0.png" alt="" aria-hidden="true" onerror="this.style.display='none'">` : '';
...
const MOB_ANIM_WEAPON_FX = new Set(['長老．艾迪爾', '長老．拉曼斯', '血色術士', '長老隨從', '魂騎士', '闇黑君王', '地獄束縛犬', '地元素守護者', '火元素守護者', '血騎士', '混沌', '死亡', '暗殺軍王史雷佛', '德雷克', '巨大墳墓守護者', '風元素守護者', '墳墓守護者騎士', '深淵之主', '深淵風靈', '火靈之主', '水靈之主', '墮落的司祭(四階)', '墮落的司祭(五階)', '魔族暗殺團', '黑暗棲林者', '法令軍王蕾雅', '黑法師', '喚獸師', '魔獸軍王巴蘭卡', '黑虎', '魔熊', '邪惡的鐮刀死神', '不死的木乃伊王', '闇黑的騎士范德', '夢幻之島殺人蜂', '恐怖的吸血鬼', '馬昆斯吸血鬼', '炎魔的分身', '象牙塔炎魔之影', '死亡之劍', '象牙塔死亡之劍', '遺忘之島巨大牛人', '變形怪首領', '夢魘', '恐怖夢魘', '象牙塔翼魔', '鋼鐵高崙', '恐怖的鋼鐵高崙', '象牙塔鋼鐵高崙', '死亡騎士', '卡瑞', '巨蟻女皇', '伊萊克頓', '希爾黛斯', '法利昂', '夢幻之島火蜥蜴', '火蜥蜴', '熔岩高崙', '伊弗利特', '恐怖的伊弗利特', '火焰烈炎獸', '烈炎獸', '暗黑火焰戰士', '火焰戰士', '不死鳥', '暗黑火焰弓箭手', '火焰弓箭手', '火焰阿西塔基奧', '阿西塔基奧', '爆彈花', '龍蠅', '巴拉卡斯', '冰之女王', '冰之女王侍女', '墳墓守護者', '哈汀之影', '長老．琪娜', '長老．巴洛斯', '墮落的司祭(二階)', '惡魔', '炎魔的惡魔', '象牙塔惡魔之影']);
...
const MOB_ANIM_WEAPON_FX2 = new Set(['德雷克', '夢幻之島殺人蜂', '伊弗利特', '恐怖的伊弗利特', '龍蠅', '冰之女王', '冰之女王侍女']);   // ⚠️冰之女王/侍女只有 death_w2(死亡爆發第二武器層)·live 恆隱藏·僅死亡殘影用；夢幻之島殺人蜂 idle_w2/attack_w2 live 顯示(第二翅膀層)
...
function _mobAnimProbe(name) {
    ...
    let pending = 6 + (hasShadow ? 6 : 0) + (hasWeapon ? 6 : 0) + (hasWeapon2 ? 6 : 0) + (hasSkillFx ? (1 + (skfCfg.endPfx ? 1 : 0) + (skfCfg.startPfx2 ? 1 : 0)) : 0);
