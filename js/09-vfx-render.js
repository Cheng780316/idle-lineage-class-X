...
const LEGEND_WEAPON_ANIM_FX = {
    ...
    '致命落雷':   { src:'assets/effects/legend-plus6/holycrystal-wand.webp?v=20260717-ground-low4', h:1.55, ar:1.0000, anchorX:0.5000, anchorY:0.9000, duration:1590, motion:'native' },
...
const _WEAPON_ANIM_FOCUS = Object.freeze({
    ...
    'holycrystal-wand.webp':   { x:0.5000, bottom:0.9000, ar:1, duration:1590 },
...
function playGodWeaponAnimatedFx(skn, mob) {
    try {
        ...
        let cfg = GOD_WEAPON_ANIM_FX[skn] || LEGEND_WEAPON_ANIM_FX[skn]; if (!cfg) return;
...
function playClientSkillAnimatedFx(skn, mob, actor) {
    try {
        ...
        if (cfg.placement === 'target') {
            ...
            if (typeof _actorVisibleRect === 'function') r = _actorVisibleRect(actor);
            else if (actor === player && typeof _pmCasterRect === 'function') r = _pmCasterRect();
...
// 🤝 玩家／傭兵共用技能特效入口：依 registry placement 決定綁目標或施法者，
// 未收錄於客戶端 WebP 時再退回既有逐幀 SPELL_FX／SELF_FX。
function playActorSkillFx(actor, sk, targets) {
    try {
        if (_vfxMute() || !sk) return false;
        let skn = typeof sk === 'string' ? sk : sk.n; if (!skn) return false;
        let list = Array.isArray(targets) ? targets : (targets ? [targets] : []);
        let cfg = (typeof CLIENT_SKILL_ANIM_FX !== 'undefined') ? CLIENT_SKILL_ANIM_FX[skn] : null;
        if (cfg && cfg.placement === 'target') {
            let played = false;
            list.forEach(t => { if (t && t.curHp > 0) { playSpellFx(skn, t); played = true; } });
            return played;
        }
        if (cfg && cfg.placement === 'caster') return playClientSkillAnimatedFx(skn, null, actor);
        if (list.length && typeof SPELL_FX !== 'undefined' && SPELL_FX[skn]) {
            list.forEach(t => { if (t && t.curHp > 0) playSpellFx(skn, t); });
            return true;
        }
        if (typeof SELF_FX !== 'undefined' && SELF_FX[skn] && typeof playSelfFx === 'function') {
            playSelfFx(skn, _actorVisibleRect(actor));
            return true;
        }
    } catch (e) {}
    return false;
...
function playTtmiSkillFx(skillName, mob, casterRect) {
    try {
        if (_vfxMute() || !mob) return;
        let cfg = TTMI_SKILL_GIF_FX[skillName]; if (!cfg) return;
        let ml = document.getElementById('mob-list');
        let slot = ml && ml.querySelector('.mob-target[data-uid="' + mob.uid + '"]');
        if (!slot) return;
        let box = slot.querySelector('.mob-img-inner') || slot.querySelector('.mob-img-wrap') || slot;
        let r = cfg.casterHead && casterRect ? casterRect : box.getBoundingClientRect();
        if (!r || !r.width || !r.height) return;
        let layer = _vfxLayer(); if (!layer || layer.childElementCount > 220) return;
        let size = cfg.size || Math.max(cfg.minSize || 96, r.height * cfg.scale);
        if (cfg.maxSize) size = Math.min(cfg.maxSize, size);
        let el = document.createElement('img');
        el.className = 'vfx-spell vfx-ttmi-skill';
        el.src = cfg.src;
        el.alt = '';
        ...
        el.style.top = cfg.casterHead
            ? (r.top - size * 0.82) + 'px'
            : (r.top + r.height * 0.48 - size * 0.5) + 'px';
        el.style.objectFit = 'contain';
        el.style.mixBlendMode = cfg.blend || 'normal';
        el.style.zIndex = '12';
        layer.appendChild(el);
        setTimeout(() => { try { el.remove(); } catch (_) {} }, cfg.duration);
    } catch (e) {}
}
...
function playShockStunHitFx(mob) {
    try {
        ...
        el.style.width = size + 'px'; el.style.height = size + 'px';
        el.style.left = (r.left + r.width * 0.5 - size * 0.5) + 'px';
        el.style.top = (r.top + r.height * 0.25 - size * 0.5) + 'px';
        el.style.objectFit = 'contain'; el.style.zIndex = '13';
        layer.appendChild(el);
        setTimeout(() => { try { el.remove(); } catch (_) {} }, 950);
    } catch (e) {}
}

// 🖤 破壞盔甲：只在狀態實際成功時，於目標身上播放來源最右欄的紅色落光／爆裂動畫。
function playArmorBreakRedFx(mob) {
    try {
        if (_vfxMute() || !mob) return;
        let ml = document.getElementById('mob-list');
        let slot = ml && ml.querySelector('.mob-target[data-uid="' + mob.uid + '"]');
        ...
        let r = box.getBoundingClientRect(); if (!r.width || !r.height) return;
        let layer = _vfxLayer(); if (!layer) return;
        let h = Math.max(120, Math.min(190, r.height * 1.35)), w = h * (174 / 262);
        let el = document.createElement('img');
        el.src = 'assets/effects/armor-break-red.webp'; el.alt = '';
        el.className = 'vfx-armor-break-red';
        el.style.position = 'fixed'; el.style.pointerEvents = 'none';
        el.style.width = w + 'px'; el.style.height = h + 'px';
        el.style.left = (r.left + r.width * 0.5 - w * 0.5) + 'px';
        el.style.top = (r.top + r.height * 0.5 - h * 0.5) + 'px';
        el.style.objectFit = 'contain'; el.style.zIndex = '13';
        layer.appendChild(el);
        setTimeout(() => { try { el.remove(); } catch (_) {} }, 2750);
    } catch (e) {}
}
...
function _partyMemberRect(who) {
    try {
        if (!who) return null;
        if (typeof _actorVisibleRect === 'function') return _actorVisibleRect(who);
...
const MORPH_BATTLE_ANIM = new Set(['克特', '卡司特王', '思克巴女皇', '死亡騎士', '炎魔', '白金法師', '白金騎士', '艾莉絲', '銀光法師', '銀光騎士', '騎士范德', '黃金法師', '黃金騎士', '黑暗法師', '黑暗騎士',
    ...
    '真死亡騎士 冥皇丹特斯', '烈焰的死亡騎士',
    '月亮騎士絲莉安', '影子騎士格立特', '鋼鐵騎士阿頓', '天鵝的騎士依詩蒂', '幸運的魔法師宙斯']);   // Lv90 五種職業變身（透明逐幀動畫）
...
function _playerBattleForm() {   // 主玩家戰場形態：變身優先 → 職業動態 → null（變身限定主玩家·隊員一律職業動畫）
    ...
    if (m) {
        // Lv.90 五款變身使用客戶端真正的三方向素材：R=d2、F=d0（背向上方怪物）、L=d6。
        // 資料夾命名比照職業動畫（R=<名>、F=<名>F、L=<名>2），domKey 不含方向，換向時只換圖不重建角色。
        let face = (typeof player !== 'undefined' && player && player._face3) || 'R';
        let folder = LV90_MORPH_BATTLE_TUNING[m]
            ? ((face === 'F') ? m + 'F' : (face === 'L') ? m + '2' : m)
            : m;
        return { key: 'morph:' + folder, domKey: 'morph:' + m, base: 'assets/morphanim/' + encodeURIComponent(folder) + '/', wpn: null, morphName: m, face: face,
            assetRev: LV90_MORPH_BATTLE_TUNING[m] ? '20260717-lv90-normalized1' : null };
    }
...
function _battleSpriteProbe(form) {
    ...
    let probeSeq = (target, key, pfx, minF) => {   // 🚀 平行探測（滑動窗口·見 _probeFramesWin）
        _probeFramesWin(i => form.base + pfx + i + '.png' + (form.assetRev ? '?v=' + form.assetRev : ''), MOB_ANIM_MAX_FRAMES, minF || 2, frames => { target[key] = frames; finish(); });
    };
...
// Lv.90 五款變身已依客戶端原始座標正規化：同一變身的三方向與全部動作共用固定畫布。
...
const LV90_MORPH_BATTLE_TUNING = Object.freeze({
    ...
    '影子騎士格立特':  { scale: 0.58, anchor: { R:[151.5,240],   F:[149.5,231.5], L:[143.5,243] } },
    ...
    '幸運的魔法師宙斯': { scale: 0.34, anchor: { R:[156,243],     F:[140,247.5],   L:[125,265] } }
});
// 🎯 共用「角色可見本體錨點」：掃描當前職業／性別／變身／動作影格的 alpha，
// 將透明畫布裁成實際可見範圍，再提供頭部、身體、腳底三個螢幕座標。
// 來源路徑與當前影格都在 cache key 內，因此玩家、傭兵與每種動作不會共用錯誤中心點。
const _actorVisibleBoundsCache = new Map();
function _actorSpriteState(actor) {
    if (!actor) return null;
    if (typeof player !== 'undefined' && actor === player) return _pmState;
    return (actor._slot != null && typeof _allySpriteStates !== 'undefined') ? _allySpriteStates[String(actor._slot)] : null;
}
function _visibleImageBounds(img) {
    try {
        if (!img || !img.complete || !img.naturalWidth || !img.naturalHeight) return null;
        let src = img.currentSrc || img.src || img.getAttribute('src') || '';
        let key = src + '|' + img.naturalWidth + 'x' + img.naturalHeight;
        if (_actorVisibleBoundsCache.has(key)) return _actorVisibleBoundsCache.get(key);
        let c = document.createElement('canvas'); c.width = img.naturalWidth; c.height = img.naturalHeight;
        let cx = c.getContext('2d', { willReadFrequently: true }); if (!cx) return null;
        cx.clearRect(0, 0, c.width, c.height); cx.drawImage(img, 0, 0);
        let px = cx.getImageData(0, 0, c.width, c.height).data;
        let minX = c.width, minY = c.height, maxX = -1, maxY = -1;
        for (let y = 0, p = 3; y < c.height; y++) {
            for (let x = 0; x < c.width; x++, p += 4) {
                if (px[p] <= 8) continue;
        ...
        // 腳底中心不能用整張可見框中心代替：武器或光效向一側伸出時，框中心會把角色本體推向反方向。
        // 取可見高度最下方 18% 的高 alpha 像素加權中位數，會落在靴子／站立點附近並排除大多數淡光粒子。
        let footY0 = Math.max(minY, maxY - Math.max(3, Math.round((maxY - minY + 1) * 0.18)));
        let footHist = new Float64Array(c.width), footTotal = 0;
        for (let y = footY0; y <= maxY; y++) for (let x = 0; x < c.width; x++) {
            let alpha = px[(y * c.width + x) * 4 + 3];
            if (alpha <= 48) continue;
            let weight = alpha * alpha;
            footHist[x] += weight; footTotal += weight;
        }
        let footPx = (minX + maxX + 1) * 0.5;
        if (footTotal > 0) {
            let acc = 0;
            for (let x = 0; x < footHist.length; x++) { acc += footHist[x]; if (acc >= footTotal * 0.5) { footPx = x + 0.5; break; } }
        }
        let out = { x0:minX / c.width, y0:minY / c.height, x1:(maxX + 1) / c.width, y1:(maxY + 1) / c.height, footX:footPx / c.width };
        _actorVisibleBoundsCache.set(key, out);
        return out;
    } catch (e) { return null; }
}
function _applyLv90MorphFrameGeometry(st, form, frame, actor) {
    let cfg = form && LV90_MORPH_BATTLE_TUNING[form.morphName];
    if (!cfg || !st || !st.el || !st.imgs || !st.imgs.bd || !frame) return false;
    let nw = frame.naturalWidth || 1, nh = frame.naturalHeight || 1, scale = cfg.scale;
    let face = (form && form.face) || 'R';
    let anchor = (cfg.anchor && (cfg.anchor[face] || cfg.anchor.R)) || [nw * 0.5, nh];
    let footCx = anchor[0] * scale;
    // 所有方向與動作共用同一來源畫布；以待機時的本體腳底固定在隊伍基準線。
    // 不再逐幀讀取 alpha 猜腳底，避免長武器、拖尾與施法光效使角色跳位。
    let bottomPad = LV90_MORPH_BASELINE - (nh - anchor[1]) * scale;
    let img = st.imgs.bd;
    // 以 important 固定最終像素尺寸，避免外層戰鬥 sprite 共用樣式把原始 3 倍畫布撐回自然大小。
    st.el.style.setProperty('width', '1px', 'important');
    img.style.setProperty('width', (nw * scale) + 'px', 'important');
    img.style.setProperty('height', (nh * scale) + 'px', 'important');
    img.style.setProperty('max-width', 'none', 'important');
    img.style.setProperty('max-height', 'none', 'important');
    img.style.setProperty('left', (-footCx) + 'px', 'important');
    img.style.setProperty('bottom', bottomPad + 'px', 'important');
    img.style.setProperty('transform', 'none', 'important');
    st.flipX = false; st.lv90Tuned = true;
    return true;
}
function _actorVisibleAnchor(actor) {
    try {
        let st = _actorSpriteState(actor), img = st && st.imgs && st.imgs.bd;
        if (!st || !st.el || !st.el.isConnected || !img) return null;
        let canvasRect = img.getBoundingClientRect();
        if (!canvasRect.width || !canvasRect.height) return null;
        let b = _visibleImageBounds(img) || { x0:0, y0:0, x1:1, y1:1 };
        if (st.flipX) b = { x0:1 - b.x1, y0:b.y0, x1:1 - b.x0, y1:b.y1, footX:b.footX == null ? null : 1 - b.footX };
        let left = canvasRect.left + canvasRect.width * b.x0;
        let top = canvasRect.top + canvasRect.height * b.y0;
        let right = canvasRect.left + canvasRect.width * b.x1;
        let bottom = canvasRect.top + canvasRect.height * b.y1;
        let width = Math.max(1, right - left), height = Math.max(1, bottom - top);
        let cx = left + width * 0.5;
        if (st.lv90Tuned) { let er = st.el.getBoundingClientRect(); cx = er.left + er.width * 0.5; }
        return {
            rect: { left, top, right, bottom, width, height },
            head: { x:cx, y:top + height * 0.08 },
            body: { x:cx, y:top + height * 0.48 },
            foot: { x:cx, y:bottom },
            action: st.act || 'idle'
        };
    } catch (e) { return null; }
}
function _actorVisibleRect(actor) { let a = _actorVisibleAnchor(actor); return a ? a.rect : null; }
// 🧝 施法者錨點：一律回傳玩家當前影格的「可見本體」矩形，不再使用整張透明畫布中央。
function _pmCasterRect() {
    return (typeof player !== 'undefined') ? _actorVisibleRect(player) : null;
}
...
function playQiguAttackSprite(actor) {
    try {
        ...
        el.style.position = 'fixed'; el.style.pointerEvents = 'none';
        el.style.width = w + 'px'; el.style.height = h + 'px';
        el.style.left = (anchor.body.x - w * (102 / 300)) + 'px';
        el.style.top = (anchor.foot.y - h * (236 / 250)) + 'px';
...
// 🏹 三重矢專屬角色動作：來源已逐幀去除地面、韓文與內建箭矢，
// 箭矢飛行仍由遊戲的 playArrowFx 三連發負責。
function playTripleCastSprite(actor) {
    try {
        let isPlayer = actor === player;
        let st = isPlayer ? _pmState : (actor && actor._slot != null ? _allySpriteStates[String(actor._slot)] : null);
        if (!st || !st.el || !st.el.isConnected || !st.imgs || !st.imgs.bd) return;
        ...
        let r = anchor.rect;
        let layer = _vfxLayer(); if (!layer) return;
        let h = Math.max(108, r.height * 1.08), w = h * (205 / 179);
        let token = Date.now() + Math.random(); st._tripleFxToken = token;
        let el = document.createElement('img');
        el.src = 'assets/effects/triple-cast.webp'; el.alt = '';
        el.className = 'vfx-triple-cast';
        el.style.position = 'fixed'; el.style.pointerEvents = 'none';
        el.style.width = w + 'px'; el.style.height = h + 'px';
        el.style.left = (anchor.body.x - w * (100 / 205)) + 'px';
        el.style.top = (anchor.foot.y - h * (170 / 179)) + 'px';
        el.style.objectFit = 'contain'; el.style.imageRendering = 'pixelated';
        el.style.zIndex = '31';
        layer.appendChild(el);
        ...
        setTimeout(() => {
            try { el.remove(); } catch (_) {}
            if (st._tripleFxToken === token) {
                st._tripleFxToken = null;
...
// 🏹 三重矢中央光圈：只播放來源 GIF 的白色魔法光圈，不替換人物，
// 不包含來源韓文、角色或內建箭矢；真正三支箭仍由 playArrowFx 播放。
function playTripleRingFx(actor) {
    try {
        ...
        let anchor = _actorVisibleAnchor(actor); if (!anchor) return;
        let r = anchor.rect;
        // 直接使用當前 skill 影格的可見本體頭部；寬透明畫布與變身留白不再參與定位。
        let layer = _vfxLayer(); if (!layer) return;
        // 客戶端原版為 104×152、7 幀；可見光圈最大寬約 84px。
        // 以可見光圈寬度縮放，再保留完整透明畫布，避免上緣光線被裁切。
        let size = Math.max(48, Math.min(64, r.height * 0.55));
...
function _pmCurActivePrio() {   // 目前「仍在播放中」動作的權重（已播完/待機→0）
    ...
    let fms = (st.act === 'attack') ? _battleAttackFrameMs(form, (player.d && player.d.aspd) || 0, seq.length)
...
function _playerMorphRemove() {
    ...
    _pmState.flipX = false; _pmState.lv90Tuned = false;
}
...
// Lv.90 素材最長有 17 幀；若沿用一般角色的 45ms 下限，0.4 秒攻擊會在中段被下一擊重設，
// 格立特永遠看不到完整的收刀段。這五款允許抽樣播放至 18ms/幀，8fps 畫面更新仍會依序取到起手、揮刀與收刀。
function _battleAttackFrameMs(form, intervalSec, seqLen) {
    if (form && LV90_MORPH_BATTLE_TUNING[form.morphName] && intervalSec > 0 && seqLen > 0) {
        return Math.max(18, Math.min(1000 / MOB_ANIM_FPS, intervalSec * 1000 / seqLen));
    }
    return _atkFrameMs(intervalSec, seqLen);
}
...
function _playerMorphApply() {   // 8fps ticker 驅動（🗡️ v3.0.67 形態＝變身優先→職業動態·key 含武器變體→換武器自動重建）
    // 在解析素材資料夾前先依目前目標更新方向，讓主角色與傭兵使用同一套面向判定。
    let currentMorph = _playerMorphName();
    if (currentMorph && LV90_MORPH_BATTLE_TUNING[currentMorph] && _pmState.el) _class3Facing(player, _pmState.el);
    ...
    } else if (_pmState.el.parentElement !== bv) bv.appendChild(_pmState.el);
    {
        ...
        _pmState.el.style.left = LV90_MORPH_BATTLE_TUNING[form.morphName]
            ? _partySpriteXs().P
            : 'calc(' + _partySpriteXs().P + ' - ' + Math.round(_pw / 2) + 'px)';
    }   // 🗡️ v3.0.71 每輪更新：站怪物格縫隙；Lv.90 以可見腳底中心直接對齊此座標
    ...
    if (LV90_MORPH_BATTLE_TUNING[form.morphName] || CLASS_ANIM_3DIR.has(player.avatar)) _class3Facing(player, _pmState.el);   // 🧭 玩家職業／Lv.90 變身共用目標面向
    ...
    if (_pmState.act) {
        ...
        if (seq) {
            let _fms = (_pmState.act === 'attack') ? _battleAttackFrameMs(form, (player.d && player.d.aspd) || 0, seq.length)   // ⚔️ v3.0.91 攻擊動畫隨攻速加速
    ...
    if (!_applyLv90MorphFrameGeometry(_pmState, form, seq[f], player)) _pmState.lv90Tuned = false;
