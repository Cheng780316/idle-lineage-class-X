function playerAttack() {
    ...
    if (player.cls === 'illusion') {
        ...
        if (_qw && !_qw.isBow && (_qw.qigu || (player.mastery === 'i_magicsword' && !isWandWeapon(_qw)))) {
            if (_qw.qigu && typeof playQiguAttackSprite === 'function') playQiguAttackSprite(player);
            qiguPlayerAttack(target, _qw); return;
        }   // 🔮 魔劍精通：排除魔杖（魔杖不轉奇古獸必中路徑）
    }
    ...
    if (result.hit) {
        ...
        result.dmg = weaponAttrNormalDamage(player, player.eq.wpn, result.dmg);
        let _attrHpBefore = target.curHp;
        target.curHp -= result.dmg;
        weaponAttrAfterNormalHit(player, player.eq.wpn, weaponAttrActualDamage(_attrHpBefore, result.dmg));
        ...
        if (target.curHp > 0 && wpn && wpn.hitEchoMagic && Math.random() * 100 < (wpn.hitEchoMagic.rate || 0)) { let _he = wpn.hitEchoMagic, _attrEchoHpBefore = target.curHp; target.curHp -= result.dmg; if (typeof terrorVisageOnDamage === 'function') terrorVisageOnDamage(target, result.dmg, 'magic'); target.justHit = _he.ele || 'magic'; target._spellHurt = true; mobWake(target); weaponAttrWaterRecover(player, weaponAttrActualDamage(_attrEchoHpBefore, result.dmg)); logCombat(`<span class="font-bold" style="color:#fb923c;text-shadow:0 0 6px #dc2626;">【爆破】</span>烈焰爆開，額外造成 ${result.dmg} 點火屬性魔法傷害。`, 'player-special'); }   // 🏺 火精靈王的爆焰：命中 10% 追加等同本擊的火魔傷；🌅 巨大骷髏視為魔法
        ...
        if (target.curHp > 0 && wpn && wpn.onHitEleDmg && (!wpn.onHitEleDmg.rate || Math.random() * 100 < wpn.onHitEleDmg.rate)) { let _oh = wpn.onHitEleDmg, _attrEleHpBefore = target.curHp; target.curHp -= _oh.dmg; target.justHit = _oh.ele; mobWake(target); weaponAttrWaterRecover(player, weaponAttrActualDamage(_attrEleHpBefore, _oh.dmg)); logCombat(`<span class="font-bold" style="color:${RELIC_ELE_COLOR[_oh.ele] || '#e2e8f0'};">附加 ${_oh.dmg} 點${RELIC_ELE_LABEL[_oh.ele] || ''}屬性傷害。</span>`, 'player-special'); }   // 🏺 rate：灰燼戰士的火焰長劍 3% 機率；無 rate→必定（幽光/冰石鎚）
...
function stormBuffTick(sk, noMageBonus) {
    ...
    targets.forEach((t, _illusionIdx) => {
        ...
        d = Math.max(1, Math.floor(d * fragileMult(t) * wpnEnFinalMult(player.eq && player.eq.wpn, true)));    // 持續型職業技能不套神話武器增幅
...
function weaponSpellProc(target, attackHit) {
    ...
    if (wpn.procUnlockEn != null && en < wpn.procUnlockEn) return;
...
function procFreeMagicSkill(t, skId, en, areaHit, sourceItem, illusionRecoverMp, attrWaterAcc) {
    ...
    if (sk.target === 'all' && !areaHit) {
        ...
        uids.forEach((uid, i) => {
            ...
            if (mob) procFreeMagicSkill(mob, skId, en, true, sourceItem, i === 0, _attrAcc);
        });
        if (sourceItem && sourceItem.type === 'wpn') weaponAttrWaterRecover(player, _attrAcc.actual);
        return;
    }
    let effMr = (t.st && t.st.mrhalf > 0) ? (t.mr / 2) : t.mr;
    let mrFactor = mrMult(effMr);
    ...
    if (total > 0) {
        ...
        if (sourceItem && sourceItem.type === 'wpn') {
            let _actual = weaponAttrActualDamage(_attrHpBefore, total);
            if (attrWaterAcc) attrWaterAcc.actual += _actual;
            else weaponAttrWaterRecover(player, _actual);
        }
        if (typeof terrorVisageOnDamage === 'function') terrorVisageOnDamage(t, total, 'magic');   // 🌅 巨大骷髏：免費觸發法術視為魔法
...
function _procWeaponSpellHit(t, sp, en, illusionRecoverMp, attrWaterAcc) {
    ...
    let _attrHpBefore = t.curHp;
    t.curHp -= d;
    if (attrWaterAcc) attrWaterAcc.actual += weaponAttrActualDamage(_attrHpBefore, d);
...
function procWeaponSpell(t, sp, en) {
    ...
    if (sp.aoe) {
        ...
        uids.forEach((uid, i) => { let mob = mapState.mobs.find(m => m && m.uid === uid && m.curHp > 0); if (mob) _procWeaponSpellHit(mob, sp, en, i === 0, _attrAcc); });
        weaponAttrWaterRecover(player, _attrAcc.actual);
        if (!state.ff) renderMobs();
        return;
    }
    let _killed = _procWeaponSpellHit(t, sp, en, undefined, _attrAcc);
    weaponAttrWaterRecover(player, _attrAcc.actual);
    if (!_killed) renderMobs();   // 擊殺時 killMob 已負責重繪
}
...
function laiaWandHitProc(t) {
    ...
    if (typeof terrorVisageOnDamage === 'function') terrorVisageOnDamage(t, d, 'magic');   // 🌅 巨大骷髏：冰裂術視為魔法
...
function _enemyPhysicalAttackInner(mob, idx, stunChance = 0, atkDmg = null, atkDb = null) {   // atkDmg/atkDb：連擊技覆寫骰子/加值（如鐮刀劍氣斬 9×3D70+99，與一般攻擊不同）
    ...
    let _titanEr = (player.skills.includes('sk_warrior_titan_bullet') && player.hp < player.mhp * titanThreshold()) ? 50 + (typeof mythTitanBulletEr === 'function' ? mythTitanBulletEr(player) : 0) : 0;   // 泰坦增幅：神話斧每3階再+1 ER
    ...
    if (hit) {
        ...
        { let _drMult = 1.0;
          ...
          if (player.statuses && player.statuses.attrEarth > 0) _drMult *= 0.85;   // 🌍 地屬性武器：非固定物理傷害 -15%
        ...
        if(player.skills.includes('sk_warrior_titan_rock') && player.hp < player.mhp * titanThreshold() && mob && mob.curHp > 0 && totalDmg > 0) {   // ⚔️ 泰坦：岩石：HP<40%(反彈精通 80%) 受一般攻擊反射相同傷害
            let _tr = Math.max(1, Math.floor(totalDmg * fragileMult(mob) * (typeof mythTitanReflectMult === 'function' ? mythTitanReflectMult(player) : 1)));
            mob.curHp -= _tr; mob.justHit = 'magic'; mobWake(mob);
            logCombat(`<span class="font-bold" style="color:#d6d3d1;text-shadow:0 0 6px #78716c;">【泰坦：岩石】</span>反射傷害，對 <span class="${getMobColor(mob.lv)}">${mob.n}</span> 造成 ${_tr} 點傷害。`, 'magic');
            if(mob.curHp <= 0) { killMob(idx); if(player.hp <= 0) killPlayer(); return; }
...
function enemyAttackAlly(mob, ally) {
    ...
    {
        let _titanEr = (ally.skills && ally.skills.includes('sk_warrior_titan_bullet') && (ally.curHp || 0) < (ally.mhp || 1) * ((ally.cls === 'warrior' && allyHasMastery(ally, 'k_rebound')) ? 0.8 : 0.4)) ? 50 + (typeof mythTitanBulletEr === 'function' ? mythTitanBulletEr(ally) : 0) : 0;   // 泰坦增幅：神話斧每3階再+1 ER
    ...
    if (ally._attrEarthTicks > 0) totalDmg = Math.floor(totalDmg * 0.85);   // 🌍 地屬性武器：非固定物理傷害 -15%
...
function applyMobMagicToAlly(mob, sk, ally) {
    ...
    if (sk.dmg) {
        ...
        if (!sk.fixedDmg && ally._attrEarthTicks > 0) dmg = Math.floor(dmg * 0.85);   // 🌍 固定傷害不受地屬性守護影響
...
function _applyMobMagicInner(mob, sk) {
    ...
    if(sk.dmg) {
        ...
        { let _drMult = 1.0;
          ...
          if (player._setFury5) _drMult *= (1 - furyRageRatio());                                                // 😡 狂怒 5/5：依失血最多 -20%
          _drMult *= avatarSelfDmgReduceMult(player);                                                            // 🔮 v3.4.45 化身（單體）：自身持有→受傷 -3%
          if (!sk.fixedDmg && player.statuses && player.statuses.attrEarth > 0) _drMult *= 0.85;                 // 🌍 地屬性武器：非固定魔法傷害 -15%
        ...
        if(player.skills.includes('sk_warrior_titan_magic') && player.hp < player.mhp * titanThreshold() && dmg > 0 && mob.curHp > 0) {   // ⚔️ 泰坦：魔法：HP<40%(反彈精通 80%) 受技能攻擊反射相同傷害
            let _tm = Math.max(1, Math.floor(dmg * fragileMult(mob) * (typeof mythTitanReflectMult === 'function' ? mythTitanReflectMult(player) : 1)));
            mob.curHp -= _tm; mob.justHit = 'magic'; mobWake(mob);
            logCombat(`<span class="font-bold" style="color:#d6d3d1;text-shadow:0 0 6px #78716c;">【泰坦：魔法】</span>反射傷害，對 <span class="${getMobColor(mob.lv)}">${mob.n}</span> 造成 ${_tm} 點傷害。`, 'magic');
            if(mob.curHp <= 0) { let _ri = mapState.mobs.findIndex(m => m && m.uid === mob.uid); if(_ri !== -1) killMob(_ri); }
...
// 野外+血盟敵人擊殺掉寶：1% 機率獲得 1 件物品（抽法同潘朵拉黑市權重 getWeightedGachaResult；詞綴走新制——只可能獲得「祝福的」1%，屬性改由象牙塔『碧恩』使用四屬性卷軸取得，遠古不再隨機產生；仍依安定值附帶強化等級）
function pledgeBonusDrop(mob) {
    ...
    let item;
    if (isEquip) {
        // 🔧 詞綴改走新制（同 gainItem/rollAffixesNew）：只可能獲得「祝福的」1%；屬性改由碧恩使用四屬性卷軸取得，遠古不再隨機產生
