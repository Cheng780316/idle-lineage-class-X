...
function castSkillInner(skId) {
    ...
    if(sk.type === 'atk' && (player.cds.atkSk <= 0 || _echoFree || _royalFreeCast)) {   // 🏅 迴響精通／👑 魔法精通：免費施放不受攻擊冷卻限制
        ...
        if (sk.slaughter) {
            ...
            let _clientSlaughterFx = (typeof playClientSkillAnimatedFx === 'function')
                && playClientSkillAnimatedFx('屠宰者', t, player);
        ...
        if (sk.weaponDmg || sk.mpDmgPct) {
            ...
            if (sk.mpDmgPct || sk.magScale) weaponAttrWaterRecover(player, weaponAttrActualDamage(_attrHpBefore, dmg));
        ...
        if(sk.dmgType === 'physical') {
            ...
            for(let h = 0; h < hits; h++) {
                if(t.curHp <= 0) break;
                if (typeof playArrowFx === 'function') playArrowFx(player, t, h * 90, skId === 'sk_elf_triple');   // 🏹 三重矢：依詩蒂變身改用客戶端新版飛箭；其餘仍用一般箭矢·錯開 90ms 快速連發
            ...
            let _attrWaterActual = 0;   // 💧 水屬性：本次施法所有目標的實際失血合計，整次只回魔一次
    ...
    let magicCritMult = isCrit ? (1 + player.d.magicCritDmg / 100) : 1.0;
            ...
            weaponAttrWaterRecover(player, _attrWaterActual);
                    ...
                    if (Math.random() < (_msB ? ((player.d.str || 0) / 60) : ((player.d.int || 0) / (_aoe ? 60 : 100)))) {
                        ...
                        if (_live.length) {
                            ...
                            _live.forEach((m, i) => {
                                ...
                                _d = illusionMagicDmg(_d, true, i === 0); let _attrHpBefore = m.curHp; m.curHp -= _d; if (typeof terrorVisageOnDamage === 'function') terrorVisageOnDamage(m, _d, 'magic'); _attrBurstActual += weaponAttrActualDamage(_attrHpBefore, _attrHpBefore - m.curHp); m.justHit = 'magic'; mobWake(m);   // 🔮 魔爆每次發動只回一次MP，5件仍逐目標生效；🌅 巨大骷髏視為魔法
                            ...
                            weaponAttrWaterRecover(player, _attrBurstActual);
                        }
                        }
                    }
                }
