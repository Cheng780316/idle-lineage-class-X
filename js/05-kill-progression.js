...
function auditTrackKill(mob) {
    ...
    let g = Math.floor((mob.exp || 0) * (1 + partyExpBonusPct() / 100) / partyExpShareCount() * getExpGainMult(player.lv) * runtimeRewardMult('exp'));   // 🤝 效率統計記主玩家實得經驗；另套平台獎勵倍率
    if (g > 0) _audit.exp += g;
    _audit.kills++;
}
...
//   網頁版由 DEFAULT_WEB_REWARD_RATES 設定；EXE 由 preload 注入專用倍率。classicDropMult／trialItemDropMult 是所有主要掉落判定的單一倍率入口。
//   經典模式現存差異：死亡損失 5% 經驗、隱藏祝福/精通/席琳、停用武器/盾/騎士特效。
function classicDropMult() { return runtimeRewardMult('drop'); }
function trialItemDropMult(id) { return runtimeRewardMult('drop'); }
...
function killMob(idx) {
    ...
    let _expShare = mob.exp * (1 + partyExpBonusPct() / 100) / partyExpShareCount() * runtimeRewardMult('exp');
    ...
    if (!_kbNoReward && Math.random() < 0.8) {
        ...
        g = Math.floor(g * runtimeRewardMult('gold') * (1 + dollFieldVal('goldBonus') / 100));   // 平台金幣倍率 × 魔法娃娃 goldBonus%（莫提斯）
        player.gold += g;
