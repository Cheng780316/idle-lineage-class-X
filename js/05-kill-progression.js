...
function auditTrackKill(mob) {
    ...
    let g = Math.floor((mob.exp || 0) * (1 + partyExpBonusPct() / 100) / partyExpShareCount() * getExpGainMult(player.lv) * runtimeRewardMult('exp'));   // 🤝 效率統計記主玩家實得經驗；EXE 另套專用經驗倍率
    if (g > 0) _audit.exp += g;
    _audit.kills++;
}
...
//   網頁版維持 1 倍；EXE 由 preload 注入專用倍率。classicDropMult／trialItemDropMult 是所有主要掉落判定的單一倍率入口。
...
function killMob(idx) {
    ...
    if (!_kbNoReward && Math.random() < 0.8) {
        ...
        g = Math.floor(g * runtimeRewardMult('gold') * (1 + dollFieldVal('goldBonus') / 100));   // EXE 專用金幣倍率 × 魔法娃娃 goldBonus%（莫提斯）
        player.gold += g;
