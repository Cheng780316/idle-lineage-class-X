...
(function () {
  ...
  function init() {
    ...
    buildModal();
    console.log('[AFK-wiki] hooks OK — 冒險地圖右側百科已啟用。');
  }
  ...
  var COMBAT_SECTIONS = [
    ...
    { t: '⚔ 經典模式（硬核挑戰）', lines: [
      ...
      '功能限制：<b>無法進行職業精通</b>（威頓村·漢）、<b>無法進入「席琳的世界」</b>。象牙塔「碧恩」的屬性強化與賦予祝福卷軸製作仍可正常使用。',
      '戰鬥簡化：經典模式<b>停用大量武器特效與被動</b>——共鳴、魔擊、連射、反擊、居合、穿透、切割、出血、鈍擊、盾牌格檔、雙刀「雙刃」、鋼爪「額外重擊」，以及騎士的看破／殺戮。一切回歸最樸素的天堂式戰鬥。'
    ]},
    { t: '🏛️ 舊「傳統模式」已取消', lines: [
      '目前只保留<b>一般模式</b>與<b>經典模式</b>。舊存檔載入時會自動遷移：一般＋傳統併入一般，經典＋傳統併入經典。',
      '遷移後可正常手動強化裝備，也可取得與使用施法卷軸；舊模式的倉庫與收藏資料會併入對應的一般／經典資料，不會要求重新創角。'
    ]},
    { t: '⌨️ 鍵盤快捷鍵（桌機）', blocks: [
      ...
      { t: 'tbl', h: ['按鍵', '作用'], rows: [
        ...
        ['Ctrl＋C', '立即返回血盟據點（未入盟會提示；石化等行動受限中、時空裂痕內無法使用；占領城池期間改回占領的城堡）']
      ]},
  ...
  var ENHANCE_SECTIONS = [
    ...
    { t: '三種詞綴（屬性／遠古／祝福，可同時各帶一個）', lines: [
      ...
      '<b>祝福／詛咒</b>：祝福武器＝額外傷害+2、命中+2、額外魔法點數+3；祝福防具＝防禦(AC) -1、減傷+1、HP+10；祝福飾品＝防禦(AC) -1、魔防+2、HP+5、MP+3。詛咒則是上述能力的負鏡像。'
    ]},
    { t: '詞綴怎麼來', lines: [
      '<b>祝福</b>：打怪／製作／血盟掉落有 1% 機率直接帶「祝福的」（在「席琳的世界」擊殺時 ×3＝3%）；強化卷軸取得時也各有 1% 變成祝福／詛咒卷軸。（黑市直接購買是「即所見」、不會附帶詞綴）',
      '<b>屬性</b>：不再由打怪或製作隨機產生。攜帶火／水／風／地之武器強化卷軸，向象牙塔「碧恩」對裝備中的武器施法；每次成功率 7%，失敗只消耗卷軸。成功會由第1階開始，同屬性最高升到第5階；改用不同屬性則從新屬性的第1階重新開始。',
      '<b>遠古系詞綴</b>：目前不再由新掉落或製作隨機產生；舊存檔已有的遠古／永恆／不朽／太初裝備仍會保留並正常生效。',
      '<b>賦予祝福卷軸</b>由象牙塔「碧恩」製作：武器＝對武器施法的卷軸×500＋席琳結晶×5＋500萬金幣（賦予成功率30%）；盔甲＝對盔甲施法的卷軸×500＋席琳結晶×5＋500萬金幣（30%）；飾品＝對飾品施法的卷軸×20＋席琳結晶×10＋1000萬金幣（40%）。製作卷軸必定成功，百分比是卷軸對裝備賦予祝福時的成功率；失敗只消耗卷軸。',
      '詛咒或已祝福的裝備不能再次賦予祝福。詛咒裝備可由碧恩優先消耗「解除詛咒的卷軸」清除；沒有卷軸時可支付 100 萬金幣。'
    ]}
  ];
  ...
  function renderMap() {
    ...
    var h = '<div class="m-wiki-note">遊戲移動方式：打開<b>地圖選單</b>→ 左邊選<b>領域</b> → 右邊選該領域的<b>地圖</b>直接傳送（受進入條件擋）。「<b>回村</b>」按鈕回到你<b>最近待過的村莊／安全區</b>（沒紀錄才回職業家鄉；占領城池期間改回占領的城堡）。下面依<b>領域</b>分組，每張圖標出<b>等級範圍</b>與<b>進入條件</b>；用搜尋（例 <b>底比斯</b>）也找得到它在哪個領域。</div>';
    ...
    if (typeof CASTLE_EXTRA !== 'undefined' && CASTLE_EXTRA.length) {
      ...
      h += '<div class="m-wiki-note" style="margin-top:0;">這些圖<b>不在一般地圖清單</b>，要<b>攻城獲勝並持續占領對應城池</b>才會開放；每日清晨 5 點守城失敗後立即關閉。攻城玩法見「血盟」分頁。</div>';
      CASTLE_EXTRA.forEach(function (v) {
        ...
        bits.push('<span class="c-mapunlock">📍 攻下並持續占領' + esc(city) + '期間開放</span>');
        if (lv) bits.push('<b style="color:#86efac;margin:0;">' + lv + '</b>');
  ...
  function renderAlly() {
    ...
    var c1 = card('① 怎麼招募', { html: kvTbl([
      ...
      ['遊戲模式', '一般／經典<b>兩組各自獨立</b>，只能招募<b>同模式</b>的存檔']
    ]) });
  ...
  function renderMode() {
    ...
    function r(item, general, classic) { return '<tr><td ' + td + '>' + item + '</td><td ' + tdc + '>' + general + '</td><td ' + tdc + '>' + classic + '</td></tr>'; }
    ...
    var note = '<div class="m-wiki-note">目前只保留<b>一般</b>與<b>經典</b>兩種模式；經典模式在創角時選擇，建立後不能更改。舊「傳統」存檔會自動合併到相對應的一般或經典資料桶。下面只列兩種模式的實際差異。</div>';
    ...
    ) + '<div class="m-wiki-desc" style="margin-top:6px;">・經典掉寶率 ×1/10 <b>不影響「職業試煉／任務道具」與「遺物」</b>，<b>卡瑞的屠龍劍</b>也維持 100% 必掉。</div>');

    var c2 = card('🛠️ 強化 · 裝備', tbl(
      r('自己手動強化（快速強化）', OK, OK) +
      r('裝備的強化值怎麼來', '自己強化', '自己強化') +
      r('施法卷軸（怪物·黑市·寶箱·兌換）', OK, OK)
    ) + '<div class="m-wiki-desc" style="margin-top:6px;">・兩種模式都能使用強化功能與施法卷軸；舊傳統模式的隨機自帶強化規則已取消。</div>');
    ...
    var c5 = card('🎴 卡片 · 裝備收集冊', tbl(
      r('卡片掉率', '100%', GOOD('同一般')) +
      r('收集冊進度共用範圍', '一般組', '經典組')
    ) + '<div class="m-wiki-desc" style="margin-top:6px;">・卡片掉率<b>不吃經典 ×1/10</b>。卡片／裝備／道具／遺物四本收集冊都<b>跟倉庫同規則</b>：一般與經典各自獨立一份。詳細玩法見「收藏-怪物」「收藏-裝備」「收藏-道具」分頁。</div>');
  ...
  function renderPledge() {
    ...
    out += wCard('🌲 風木城專屬：風木地監',
      wDesc('攻下並持續占領<b>風木城</b>期間，會開放「風木地監」狩獵圖（肯特、海音沒有）。怪有殭屍／骷髏／史巴托／食屍鬼／食人魔（含王）／地獄犬，BOSS <b>巴列斯</b>。') +
      wDesc('巴列斯掉「失去魔力的巴列斯魔杖」，配靈魂之球可喚醒成傳說武器<b>巴列斯魔杖</b>（見「裝備」「任務」）。完整怪表搜「掉落查詢→風木地監」。清晨 5 點守城失敗後立即關閉。'));
