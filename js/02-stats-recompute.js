...
function recomputeStats() {
    ...
    if (p.eq.wpn) {
        ...
        // 祝福的武器：額外傷害+2、額外魔法點數+3、額外命中+2
        applyBlessStats(d, p.eq.wpn.bless, 'wpn', p);   // 祝福的/詛咒的
        // 遠古武器：額外傷害+2、魔法傷害+1
    ...
    if (p.eq.offwpn) {
        applyBlessStats(d, p.eq.offwpn.bless, 'wpn', p); applyAncStats(d, p.eq.offwpn.anc, 'wpn');
...
d.mr += (baseMr + bonusMr);
        ...
        // 祝福的：防具→AC-1、傷害減免+1、HP+10；飾品→AC-1、MR+2、HP+5、MP+3
        applyBlessStats(d, e.bless, (ed.slot==='ring'||ed.slot==='amulet'||ed.slot==='belt'||ed.slot==='ear') ? 'acc' : 'arm', p);   // 祝福的/詛咒的
    ...
    if(p.statuses && p.statuses.attrWind > 0) spdMult *= (1 / 1.2);   // 🌪️ 風屬性武器：觸發後攻速+20%（重複觸發只刷新時間）
    ...
    // 套裝預設優先；80級或持有控制戒指時，計時中的卷軸變身可覆蓋套裝。武器強制變身不受此解鎖影響。
...
function playerMoveDelayMultiplier() {
    let p = player || {};
    let buffs = p.buffs || {};
    let _pfT = (typeof getActivePolyForm === 'function') ? getActivePolyForm(p)
             : (p._setPoly || ((buffs.poly > 0 && p.poly) ? p.poly : null));
    let pfW = (_pfT && _pfT.wlk) ? _pfT.wlk : 16;
