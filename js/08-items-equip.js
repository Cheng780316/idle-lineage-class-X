...
function findOwnedEquipment(targetUid){return player.inv.find(i=>i.uid===targetUid)||Object.values(player.eq||{}).find(i=>i&&i.uid===targetUid);}
function findScrollItem(itemId){return player.inv.find(i=>i.id===itemId&&(i.cnt||1)>0);}
function useMysticScrollOnEquipment(targetUid){let s=findScrollItem('scroll_mystic_enchant');if(!s){logSys('<span class="text-red-300">沒有裝備魔力附魔卷軸。</span>');return;}activeMysticScroll=s;doMysticEnchant(targetUid);}
function useProtectScrollOnEquipment(targetUid){let s=findScrollItem('scroll_enhance_protect');if(!s){logSys('<span class="text-red-300">沒有武器裝備強化保護卷軸。</span>');return;}activeProtectScroll=s;applyEnhanceProtect(targetUid);}
...
function doMysticEnchant(targetUid){let s=activeMysticScroll,t=findOwnedEquipment(targetUid);if(!s||!t)return;let d=DB.items[t.id],isEquipped=!player.inv.includes(t);if(!isEquipped&&(t.cnt||1)>1){t.cnt--;t={...t,cnt:1,uid:uid(),lock:false,junk:false};player.inv.push(t);}consume(s);t.mystic=rollMysticEquipment(d,t.id);t.identified=true;logSys(`<span class="text-fuchsia-300 font-bold">附魔完成：${getItemFullName(t)}</span>`);activeMysticScroll=null;renderTabs(true);calcStats();saveGame();let slot=isEquipped?Object.keys(player.eq||{}).find(k=>player.eq[k]===t):undefined;openModal(t,isEquipped,slot);}
...
function applyEnhanceProtect(targetUid){let s=activeProtectScroll,t=findOwnedEquipment(targetUid);if(!s||!t||t.enhanceProtect)return;let isEquipped=!player.inv.includes(t);consume(s);t.enhanceProtect=1;activeProtectScroll=null;logSys(`<span class="text-cyan-300 font-bold">${getItemFullName(t)} 已獲得 1 次強化保護。</span>`);renderTabs(true);saveGame();let slot=isEquipped?Object.keys(player.eq||{}).find(k=>player.eq[k]===t):undefined;openModal(t,isEquipped,slot);}
