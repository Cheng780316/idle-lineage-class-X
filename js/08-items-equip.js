...
function useItem(u, silent = false) {
    ...
    if (d.type === 'pot' || d.eff === 'poly' || d.eff === 'reset' || d.eff === 'magicbarrier' || d.eff === 'teleport_scroll' || d.eff === 'panacea') {   // 變形卷軸(eff:poly)、回憶蠟燭(eff:reset)、魔法卷軸(eff:magicbarrier)、瞬間移動卷軸(eff:teleport_scroll)亦走此消耗品分支
        ...
        if (d.type === 'pot' && d.val != null) {
            ...
            if (silent && freePoly && player.poly && polyFormMatchesEquippedWeapon(player.poly)) {
                // 自動使用 + 自由變身：維持上次選定狀態（不重抽、不跳選單）
                // 保留 player.poly 不變，僅於下方重置持續時間。
            } else {
                // 其餘情況（未滿80且無戒指／尚無紀錄／換成不同攻擊類型武器）：依等級與武器類型隨機抽取。
...
function checkCanEquip(item) {
    ...
    if (d && d.godWeapon) return reqAllowsClass(d, player.cls);   // ✨ 神話武器嚴格依單一 req 職業，不受武器標籤、職業白名單或精通放寬
...
function renderStatusIconBar() {
    ...
    if(bar.dataset.statusSig!==sig){
        ...
        bar.innerHTML=rows.map((x,i)=>{let title=x.label+(x.ticks>0?'｜剩餘 '+x.sec+' 秒':'');let iconName=String(x.icon||x.name);let iconFile=/\.(?:png|jpe?g|webp|gif)$/i.test(iconName)?iconName:(iconName+'.jpg');return `<div class="status-icon${x.ally?' status-icon--ally':''}" data-status-index="${i}" title="${title}"><img src="assets/state-icons/${encodeURIComponent(iconFile)}" alt="${x.label}"></div>`;}).join('');
    } else {
...
function _updateUIImpl() {
    ...
    { let vb = document.getElementById('victory-badge'); if (vb) { let _va = siegeVictoryActive(); vb.style.display = _va ? 'inline-flex' : 'none'; if (_va) vb.title = `占領城池期間：全商店8折、開放${victoryCityCfg().castleName}`; } }   // 城池占領淡金黃標記（inline-flex 讓👑與文字水平置中；tooltip 依實際占領城池動態）
