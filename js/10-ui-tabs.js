...
// ✨ 神話武器一職一把：覆寫為單一職業白名單，避免其他武器標籤或精通放寬限制。
const GOD_WEAPON_CLASS = {
    god_royal_flash: 'royal', god_knight_judgment: 'knight', god_elf_obsession: 'elf', god_mage_eva: 'mage',
...
Object.keys(DB.items).forEach(function(id) {
    let d = DB.items[id];
    if (!d || !d.godWeapon) return;
    if (d.qigu) delete d.ignHardSkin;
    else d.ignHardSkin = true;
});
...
function buildItemDescHTML(item) {
    ...
    if (d.type === 'wpn' || d.type === 'arm' || d.type === 'acc') {
        ...
        if (d.procSkill) {
            ...
            let _procLabel = d.procOnHit ? '命中施法' : '攻擊施法';
            if (d.procUnlockEn != null && capWpnEn(item.en || 0) < d.procUnlockEn) _eff.push(`${_procLabel}：尚未解鎖（+${d.procUnlockEn}起固定${d.procRateBase || 1}%觸發${_procName}）`);
            else _eff.push(`${_procLabel} ${_procRate}%（觸發${_procName}）`);
        }
    ...
    if (d.type === 'wpn' || d.type === 'arm' || d.type === 'acc') {
        ...
        let _logos = _EQ_CLASSES
            .filter(([c]) => (d.godWeapon || isRelic(d)) ? reqAllowsClass(d, c) : (c === 'dark') ? darkEquipOk(d, item.id) : (c === 'illusion') ? illusionEquipOk(d, item.id) : (c === 'dragon') ? dragonEquipOk(d, item.id) : (c === 'warrior') ? warriorEquipOk(d, item.id) : (c === 'royal') ? royalEquipOk(d, item.id) : reqAllowsClass(d, c))   // ✨ 神話／🏺遺物：職業圖示純以 req 白名單判定，與 checkCanEquip 短路一致
...
function showEnhanceOptions(uid, isEq) {
    ...
    document.getElementById('modal-item-desc').innerHTML = "請選擇要使用的卷軸：" + _protectHint;
    
    let act = '';
    if (scrollNorm || scrollBless || scrollCurse) act += `<div class="col-span-2 text-sm font-bold text-slate-300 border-b border-slate-700 pb-1">裝備強化</div>`;
    if (scrollNorm) {
...
function activateEquipProtectFromEnhance(scrollUid, targetUid, isEq) {
    ...
    let td = target && DB.items[target.id];
    ...
    if (!canUseEquipProtectState(td, target.en, kind)) {
        logSys('<span class="text-red-400 font-bold">祝福的 裝備保護卷軸僅能用於 +11 以上武器或 +9 以上防具。</span>');
        return;
    }
    player.equipProtect = kind;
    consume(scroll);
